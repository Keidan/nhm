...
auto QNHMRule::copy(void* api_rule) -> void
{
...
auto QNHMRule::toStrings() -> QStringList
{
  QStringList list;
...
auto QNHMRule::toString(int column) -> QString
{
  QString str;
  switch(column)
  {
...
auto QNHMRule::fromString(int column, const QString &value) -> void
{
  unsigned int imac[6];
  struct in6_addr in6;
  QString str;
  switch(column)
  {
    ...
    {
      if(value == "DROP") 
        nf_type = NHM_NF_TYPE_DROP;
      else if(value == "ACCEPT") 
        nf_type = NHM_NF_TYPE_ACCEPT;
      else if(value == "STOLEN") 
        nf_type = NHM_NF_TYPE_STOLEN;
      else if(value == "QUEUE") 
        nf_type = NHM_NF_TYPE_QUEUE;
      else if(value == "REPEAT") 
        nf_type = NHM_NF_TYPE_REPEAT;
      else if(value == "STOP") 
        nf_type = NHM_NF_TYPE_STOP;
      break;
    }
    case COLUMN_DIR:
    {
    ...
    case COLUMN_PROTO:
    {
      ...
      {
        auto ok = false;
        auto n = value.left(idx).toInt(&ok, 10);
        if(ok && n != eth_proto)
        {
          eth_proto = n;
          ip_proto = 0;
    ...
    case COLUMN_APPLIED:
    {
      auto idx = value.indexOf(" - ");
      auto ok = false;
      auto l = value.left(idx).toLong(&ok, 10);
      if(ok && ((unsigned long)l) != counter) 
        counter = l;
      ok = false;
      str = value.right(value.size() - (idx + 3));
      idx = str.indexOf(".");
