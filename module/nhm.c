...
#include <nhm.h>
#if (LINUX_KERNEL_VERSION >= KERNEL_VERSION(4, 13, 0))
...
static int __init nhm_init(void)
{
  ...
  while(dev)
  {
    printk(KERN_INFO "[NHM] Net_Device found: name='%s', ifindex=%d\n", dev->name, dev->ifindex);
...
static void __exit nhm_exit(void)
{
  ...
  nhm_list_clear();
  /* remove the device */
  device_destroy(nhm_class, MKDEV(NHM_MAJOR_NUMBER, 0));
  /* unregister the device class */
  class_unregister(nhm_class);
...
static void nhm_print_rule(const char* title, struct nhm_s *rule)
{
  ...
  printk(KERN_INFO "[NHM] %s dev '%s' dir: %s\n", title, rule->dev,
...
{
  if(nhm_nf_type == NHM_NF_TYPE_ACCEPT)
    return NF_ACCEPT;
  else if(nhm_nf_type == NHM_NF_TYPE_DROP)
    return NF_DROP;
  else if(nhm_nf_type == NHM_NF_TYPE_STOLEN)
    return NF_STOLEN;
  else if(nhm_nf_type == NHM_NF_TYPE_QUEUE)
    return NF_QUEUE;
  else if(nhm_nf_type == NHM_NF_TYPE_REPEAT)
    return NF_REPEAT;
  return NF_STOP;
}
...
/**
 * @brief A module must use the module_init() module_exit()
 * macros from linux/init.h, which identify the initialization function
 * at insertion time and the cleanup function (as listed above)
 */
